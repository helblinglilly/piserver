...
export default function EnergyBillAdd(){
  ...
  const [electricityStandingChargeRate, setElectricityStandingChargeRate] = useState(0);
  const [electricityCost, setElectricityCost] = useState<number | undefined>();
  const [electricityCharged, setElectricityCharged] = useState<number | undefined>();
  ...
  useEffect(() => {
    const fetchLatestBillEndDate = async () => {
      ...
      if (result.status !== 200){
        setFailureMessages([...failureMessages, "Failed to fetch latest bill end date."])
        return;
      }
  ...
  const handleSubmitClick = async () => {
    const {isValid, messages} = validateBillInputFE(standingChargeDays, electricityUsage, electricityRate, electricityStandingChargeRate, electricityCharged, electricityCost, gasUsage, gasRate, gasStandingChargeRate, gasCost, gasCharged);
  ...
  <Notification message={failureMessages} type="fail" />
    ...
    <div className="columns">
      <div className="column">
        ...
        {
          lastBillEndDate && startDate ? <DatePicker 
          changeHandler={(date) => setStartDate(date)} 
          name={"startDate"} 
          initialDate={startDate} 
          minDate={lastBillEndDate} 
          maxDate={endDate} /> : <input className="input loading" type="date"/>
        }
        </div>
      <div className="column">
      ...
      <div className="column">
        ...
        {
          lastBillEndDate 
          ? <input className="input" type="number" value={standingChargeDays} disabled readOnly/> 
          : <input className="input loading" type="date"/>
        }
    ...
    <BillInput 
      ...
      <button className="button is-info" style={{width: "100%", marginTop: "20px"}} onClick={handleSubmitClick}>Submit</button>
  </>)
}