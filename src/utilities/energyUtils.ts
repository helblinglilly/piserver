export const validateBillInputFE = (
	standingChargeDays: number,
	electricityUsage: number,
	electricityRate: number,
	electricityStandingChargeRate: number,
	electricityCharged: number | undefined,
	electricityCost: number | undefined,
	gasUsage: number,
	gasRate: number,
	gasStandingChargeRate: number,
	gasCost: number | undefined,
	gasCharged: number | undefined,
) => {
	let isValid = true;
	...
	if (electricityUsage > 2500) {
		messages.push(
			"Electricity usage was above 2500kWh - are your readings correct?",
		);
		isValid = false;
	}
	if (electricityRate > 1000) {
		messages.push(
			"Electricity unit rate was above £1/kWh - did you input pence?",
		);
		isValid = false;
	}
	...
	if (electricityStandingChargeRate > 1000) {
		messages.push(
			"Electricity standing charge rate was above £1/day - are you sure?",
		);
		isValid = false;
	}
	...
	if (!electricityCharged || electricityCharged <= 0) {
		...
		if (
			electricityCharged > electricityCost * 1.5 ||
			electricityCharged < electricityCost * 0.5
		) {
			messages.push(
				"Electricity amount charged varies by more than 50% from calculated cost - are you sure?",
			);
			isValid = false;
		}
	}
	...
	if (!gasCharged || gasCharged === 0) {
		...
		if (gasCharged > gasCost * 1.5 || gasCharged < gasCost * 0.5) {
			messages.push(
				"Gas amount charged varies by more than 50% from calculated cost - are you sure?",
			);
			isValid = false;
		}
	}
...
export const validateBillInputBE = (
	usage: number,
	rate: number,
	...
	if (charged <= 0) {
		...
		if (charged > cost * 1.5 || charged < cost * 0.5) {
			messages.push(
				"Charged amount varies by more than 50% from calculated cost - are you sure?",
			);
			isValid = false;
		}
	}
