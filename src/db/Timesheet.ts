import { date, pgTable, primaryKey, timestamp, varchar } from "drizzle-orm/pg-core";
...
export async function setBreakIn(username: string, day: Date, breakIn: Date){
	...
	await db.insert(TimesheetBreaks).values({
		...
		{
			target: [TimesheetBreaks.username, TimesheetBreaks.date, TimesheetBreaks.breakIn], 
			set: { breakIn: breakIn }, 
			where: and(
...
export async function insertBreakOut(username: string, day: Date, breakOut: Date){
	...
	await db.update(TimesheetBreaks).set({
		breakOut: breakOut,
	}).where(and(
		eq(TimesheetBreaks.username, username),
		eq(TimesheetBreaks.date, normalisedDay.toISOString()),
...
export async function getTimesheet(username: string, day: Date): Promise<ITimesheet | undefined> {
	...
	const timesheetQuery = db.select({ clockIn: Timesheet.clockIn, clockOut: Timesheet.clockOut })
		.from(Timesheet)
		.where(
			and(
				eq(Timesheet.username, username),
	...
	const breakQuery = db.select({ breakIn: TimesheetBreaks.breakIn, breakOut: TimesheetBreaks.breakOut }).from(TimesheetBreaks).where(
		and(
			eq(TimesheetBreaks.username, username),
			eq(TimesheetBreaks.date, normalisedDay.toISOString())
		)
