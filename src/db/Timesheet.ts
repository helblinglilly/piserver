...
export async function getTimesheet(username: string, day: Date): Promise<ITimesheet | undefined> {
	...
	try {
		return await db.all(`SELECT timesheet.clock_in, timesheet.clock_out, timesheet_breaks.break_in, timesheet_breaks.break_out
            FROM timesheet
